...
def main(cfg) -> None:
    ...
    for user_behavior in cfg.user_behavior:
        ...
        result_df_list = []
        for beta in cfg.variation.eps_list:
            ...
            message = f"behavior={user_behavior}, beta={beta}"
            ...
            result_df = aggregate_simulation_results(
                simulation_result_list=result_list, policy_value=policy_value, x_value=beta
            )
        ...
        for yscale in ["linear", "log"]:
            for is_only_mse in [True, False]:
                img_path = result_path / f"{yscale}_mse_only={is_only_mse}_varying=beta_{user_behavior}.png"
