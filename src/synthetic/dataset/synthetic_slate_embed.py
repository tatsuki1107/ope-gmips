...
class SyntheticSlateDatasetWithActionEmbeds(BaseBanditDataset):
    ...
    def __post_init__(self) -> None:
        ...
        self.obs_category_idx = np.arange(self.n_cat_dim)[self.n_unobserved_cat_dim :]
        self.action_at_k_idx = np.arange(self.n_actions_at_k)[:, None, None]
    ...
    def aggregate_propensity_score(
        ...
        reshaped_slate_embed = slate_action_context[:, :, None, None]
        ...
        for pos_ in position:
            p_e_i_d_a_k = p_e_d_a_k[pos_, self.action_at_k_idx, reshaped_slate_embed[:, pos_], self.obs_category_idx]
            p_e_i_d_a_k = p_e_i_d_a_k.squeeze(axis=2)

            # p(e|a) = \prod_{d=1}^{D} p(e(d)|a)
            p_e_a_k = p_e_i_d_a_k.prod(axis=2)

            marginal_pscore[:, pos_] = (pi_k[:, :, pos_] * p_e_a_k).sum(1)

            # for d in range(p_e_d_a_k.shape[-1]):
