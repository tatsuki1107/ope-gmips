...
def main(cfg) -> None:
    ...
    for user_behavior in cfg.user_behavior:
        ...
        result_df_list = []
        for reward_noise in cfg.variation.reward_noise_list:
            ...
            message = f"behavior={user_behavior}, reward_noise={reward_noise}"
            ...
            result_df = aggregate_simulation_results(
                simulation_result_list=result_list, policy_value=policy_value, x_value=reward_noise
            )
        ...
        for yscale in ["linear", "log"]:
            for is_only_mse in [True, False]:
                img_path = result_path / f"{yscale}_mse_only={is_only_mse}_varying=reward_noise_{user_behavior}.png"
                visualize_mean_squared_error(
                    result_df=result_df,
                    xlabel="varying the reward noise",
                    img_path=img_path,
                    yscale=yscale,
                    xscale="linear",
